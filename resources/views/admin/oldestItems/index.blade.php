...
@section('content')
@can('oldest_item_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.oldest-items.create') }}">
                {{ trans('global.add') }} {{ trans('cruds.oldestItem.title_singular') }}
            </a>
        </div>
    </div>
@endcan
<div class="card">
    <div class="card-header">
        {{ trans('cruds.oldestItem.title_singular') }} {{ trans('global.list') }}
    </div>

    <div class="card-body">
        <table class=" table table-bordered table-striped table-hover ajaxTable datatable datatable-OldestItem">
            <thead>
                <tr>
                    <th width="10">
                    ...
                    <td>
                        <select class="search">
                            ...
                            @foreach($receipt_notes as $key => $item)
                                <option value="{{ $item->expiry_date }}">{{ $item->expiry_date }}</option>
                            @endforeach
                        </select>
                    </td>
                    <td>
...
@can('oldest_item_delete')
  let deleteButtonTrans = '{{ trans('global.datatables.delete') }}';
  let deleteButton = {
    text: deleteButtonTrans,
    url: "{{ route('admin.oldest-items.massDestroy') }}",
...
@endcan
  ...
  let dtOverrideGlobals = {
    ...
    retrieve: true,
    aaSorting: [],
    ajax: "{{ route('admin.oldest-items.index') }}",
    columns: [
