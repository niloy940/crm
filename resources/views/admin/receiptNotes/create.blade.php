    ...
    <div class="card">
        ...
        <div class="card-body">
                ...
                {{-- <div>
                    <div class="form-group">
                        <label class="required"
                            for="products">{{ trans('cruds.receiptNote.fields.product') }}</label>
                        ...
                        <select class="form-control select2 {{ $errors->has('products') ? 'is-invalid' : '' }}"
                            ...
                            @foreach ($products as $id => $product)
                                <option value="{{ $id }}"
                                    {{ in_array($id, old('products', [])) ? 'selected' : '' }}>{{ $product }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group">
                        <label class="required"
                            for="quantity">{{ trans('cruds.receiptNote.fields.quantity') }}</label>
                        <input class="form-control {{ $errors->has('quantity') ? 'is-invalid' : '' }}" type="number"
                            name="quantity" id="quantity" value="{{ old('quantity', '') }}" step="0.001" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('lot') ? 'is-invalid' : '' }}" type="text" name="lot"
                    id="lot" value="{{ old('lot', '') }}" required>
                @if ($errors->has('lot'))
                    <div class="invalid-feedback">
                        {{ $errors->first('lot') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('int_lot') ? 'is-invalid' : '' }}" type="text"
                    name="int_lot" id="int_lot" value="{{ $int_lot }}" required>
            ...
            <div class="form-group">
                <label class="required"
                    for="expiry_date">{{ trans('cruds.receiptNote.fields.expiry_date') }}</label>
                <input class="form-control date {{ $errors->has('expiry_date') ? 'is-invalid' : '' }}" type="text"
            ...
            <div class="form-group">
                <label class="required"
                    for="warehouse_id">{{ trans('cruds.receiptNote.fields.warehouse') }}</label>
                <select class="form-control select2 {{ $errors->has('warehouse') ? 'is-invalid' : '' }}"
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('qc') ? 'is-invalid' : '' }}" name="qc" id="qc" required>
                    <option value disabled {{ old('qc', null) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}</option>
                    @foreach (App\Models\ReceiptNote::QC_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('qc', '') === (string) $key ? 'selected' : '' }}>
                            {{ $label }}</option>
                    @endforeach
                </select>
                @if ($errors->has('qc'))
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('conditions') ? 'is-invalid' : '' }}" name="conditions"
                    id="conditions" required>
                    <option value disabled {{ old('conditions', null) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}</option>
                    @foreach (App\Models\ReceiptNote::CONDITIONS_SELECT as $key => $label)
                        <option value="{{ $key }}"
                            {{ old('conditions', 'Čuvati na suvom i tamnom mestu.') === (string) $key ? 'selected' : '' }}>
                            {{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('shift') ? 'is-invalid' : '' }}" name="shift" id="shift"
                    required>
                    <option value disabled {{ old('shift', null) === null ? 'selected' : '' }}>
                        {{ trans('global.pleaseSelect') }}</option>
                    @foreach (App\Models\ReceiptNote::SHIFT_SELECT as $key => $label)
                        <option value="{{ $key }}"
                            {{ old('shift', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if ($errors->has('shift'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('place') ? 'is-invalid' : '' }}" type="text" name="place"
                    id="place" value="{{ old('place', 'Banatski Karlovac') }}" required>
                @if ($errors->has('place'))
                    <div class="invalid-feedback">
                        {{ $errors->first('place') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('driver') ? 'is-invalid' : '' }}" type="text"
                    name="driver" id="driver" value="{{ old('driver', '') }}" required>
            ...
            <div class="form-group">
                <label class="required"
                    for="id_driver">{{ trans('cruds.receiptNote.fields.id_driver') }}</label>
                <input class="form-control {{ $errors->has('id_driver') ? 'is-invalid' : '' }}" type="text"
                    name="id_driver" id="id_driver" value="{{ old('id_driver', '') }}" required>
            ...
            <div class="form-group">
                <label class="required"
                    for="registration">{{ trans('cruds.receiptNote.fields.registration') }}</label>
                <input class="form-control {{ $errors->has('registration') ? 'is-invalid' : '' }}" type="text"
                    name="registration" id="registration" value="{{ old('registration', '') }}" required>
